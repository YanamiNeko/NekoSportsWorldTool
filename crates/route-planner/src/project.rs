...
pub fn add_virtual_nodes(g: &mut RoadGraph, queries: &[Coord]) -> Result<Vec<NodeIndex>, String> {
    ...
    let mut cache: std::collections::HashMap<(i64, i64), NodeIndex> = std::collections::HashMap::new();
    ...
    for q in queries {
        ...
        g.add_edge(s, v, EdgeData { length_m: len_sv, ..data.clone() });
        g.add_edge(v, tg, EdgeData { length_m: len_vt, ..data.clone() });
        ...
        if !data.oneway {
            if let Some(rev) = g.graph.find_edge(tg, s) {
                ...
                g.add_edge(tg, v, EdgeData { length_m: len_tgv, ..rdata.clone() });
                g.add_edge(v, s, EdgeData { length_m: len_vs, ..rdata });
            }
        }
