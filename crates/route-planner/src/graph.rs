...
impl RoadGraph {
    ...
    pub fn rebuild_index(&mut self) {
        ...
        for e in self.graph.edge_indices() {
            ...
            let (pa, pb) = (self.to_m(self.graph[a].coord), self.to_m(self.graph[b].coord));
            items.push(Seg { a: pa, b: pb, edge: e.index() });
        }
    ...
    pub fn retain_inside_any(&mut self, polygons: &[Vec<Coord>]) {
        ...
        self.graph.retain_nodes(|g, idx| {
            polygons.iter().any(|p| point_in_polygon(p, g[idx].coord))
        });
        self.compute_anchor();
        self.rebuild_index();
    }
