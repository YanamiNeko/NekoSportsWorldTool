...
impl<T: 'static> Drop for EventLoop<T> {
    fn drop(&mut self) {
        // A new NativeActivity gets its own android_main and event loop.
        ...
        F: FnMut(event::Event<T>, &RootAEL),
    {
        ...
        if let Some(event) = main_event {
            ...
            match event {
                ...
                MainEvent::Destroy => {
                    // NativeActivity onDestroy waits for android_main to return.
